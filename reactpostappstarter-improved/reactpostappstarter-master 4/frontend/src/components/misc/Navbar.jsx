...
import ActionToggle from "./ActionToggle"; // Import ActionToggle
    ...
    <header className={classes.header}>
      <Container size="md" className={classes.inner}>
        ...
        <Group gap={5} visibleFrom="xs">
          {items}
          <ActionToggle /> {/* Include the ActionToggle component */}
        </Group>
        <Burger hiddenFrom="xs" opened={opened} onClick={toggle} />
        <Drawer withCloseButton={true} opened={opened} size="100%" onClose={toggle}>
          <Stack>{items}</Stack>
        </Drawer>
      </Container>
    </header>
